...
export default function DokumentasiPage() {
    ...
    const handleUpload = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await authFetch(apiUrl('/api/documentation'), {
                method: 'POST',
                body: formData, // authFetch will handle multipart/form-data if body is FormData
            });
        ...
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
                    ...
                    <p className="text-slate-500 font-medium">Belum ada dokumentasi ditemukan</p>
                    ...
                    {filteredDocs.map(doc => (
                        ...
                        >
                            ...
                            <div className="p-4">
                                ...
                                {doc.activity_title && (
                                    <p className="text-xs text-emerald-600 font-medium mt-1">{doc.activity_title}</p>
                                )}
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        ...
                        <form onSubmit={handleUpload} className="p-6 space-y-4">
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-slate-500 uppercase mb-1.5">Pilih File ({fileType === 'photo' ? 'Gambar' : 'MP4'})</label>
                                <div className="relative group">
                                    <input 
                                        type="file" 
                                        accept={fileType === 'photo' ? 'image/*' : 'video/mp4'}
                                        onChange={(e) => setFile(e.target.files?.[0] || null)}
                                        className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                                    />
                                    <div className={`w-full py-8 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center transition-all ${file ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 group-hover:border-emerald-400 group-hover:bg-slate-50'}`}>
                                        <Upload className={file ? 'text-emerald-600' : 'text-slate-400'} size={32} />
                                        <p className={`text-sm mt-2 font-medium ${file ? 'text-emerald-700' : 'text-slate-500'}`}>
                                            {file ? file.name : 'Klik atau seret file ke sini'}
                                        </p>
                                        <p className="text-[10px] text-slate-400 mt-1">Maks. 50MB</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <button 
                                type="submit"
                                disabled={uploadLoading || !file || !title}
            ...
            {isViewerOpen && selectedDoc && (
                <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/95 backdrop-blur-md">
                    <button 
                        ...
                        className="absolute top-6 right-6 p-3 bg-white/10 hover:bg-white/20 text-white rounded-full transition-all z-[80]"
                    >
                        <X size={24} />
                    </button>
